\documentclass[final]{beamer}

% =========================================================
% Preamble - Load all settings from separate files
% =========================================================
\input{preamble/packages}
\input{preamble/fonts}
\input{preamble/colors}
\input{preamble/header}

% =========================================================
% Document Body - Redesigned for clarity and visual flow
% Following TRB Poster Guidelines: 3-4 main concepts,
% 50% text / 50% graphics, clear left-to-right flow
% =========================================================
\begin{document}
\begin{frame}{}
    \vspace{-0.5cm}
    
    % Three Column Layout with clear narrative flow
    \begin{columns}[t]
        
        % ---------------------------------------------------------
        % COLUMN 1: THE PROBLEM (Why does this matter?)
        % ---------------------------------------------------------
        \begin{column}{0.32\paperwidth}
            
            \input{sections/sec01_problem_overview}
            \vspace{0.2cm}
            
            \input{sections/sec02_research_gap}
            
        \end{column}

        % ---------------------------------------------------------
        % COLUMN 2: THE SOLUTION (What did we do?)
        % ---------------------------------------------------------
        \begin{column}{0.32\paperwidth}
            
            \input{sections/sec03_approach}
            \vspace{0.2cm}
            
            \input{sections/sec04_key_innovation}
            
        \end{column}

        % ---------------------------------------------------------
        % COLUMN 3: THE IMPACT (What did we find?)
        % ---------------------------------------------------------
        \begin{column}{0.32\paperwidth}

            \input{sections/sec05_results}
            \vspace{0.2cm}

            \input{sections/sec06_takeaways}

        \end{column}
    \end{columns}
\end{frame}
\end{document}
